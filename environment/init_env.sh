...
install_python3()
{
	...
	if [ -z "${python_versions}" ];then
        execute_cmd "wget  http://cdn.npm.taobao.org/dist/python/$version/Python-$version.tgz -P ~/.pyenv/cache/ "
        execute_cmd"rm -rf  /root/.pyenv/plugins/python-build/share/python-build/3.7.3 "
        execute_cmd"cp /usr/local/src/3.7.3  /root/.pyenv/plugins/python-build/share/python-build/ "
        execute_cmd" pyenv install $version"
    fi
